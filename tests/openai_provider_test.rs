...
#[test]
fn test_openai_provider_integration() {
    ...
    let api_key = match env::var("RIZZLER_OPENAI_API_KEY") {
        Ok(key) => key,
        Err(_) => {
            println!("Skipping OpenAI integration test - no API key");
            return;
        }
    };
    
    if api_key.is_empty() {
        println!("Skipping OpenAI integration test - empty API key");
        return;
    }
    ...
    // Create provider
    let provider = OpenAIProvider::new().expect("Failed to create OpenAI provider");
    ...
    // Resolve the entire file first
    let file_result = provider.resolve_file(&conflict_file);
    
    match file_result {
        Ok(response) => {
            println!("Successfully resolved entire file, token usage: {:?}", response.token_usage);
            ...
            // Write the resolved file to a temporary location for inspection
            let resolved_path = format!("{}.resolved", file_path);
            File::create(&resolved_path)
                .and_then(|mut file| file.write_all(response.content.as_bytes()))
                .unwrap_or_else(|_| println!("Failed to write resolved file"));
                
            println!("Resolved file written to {}", resolved_path);
        },
        Err(e) => {
            println!("Warning: Failed to resolve entire file: {:?}", e);
            // Continue with individual conflicts
        }
    }
    
    // Also resolve individual conflicts as a fallback approach
    ...
    if Path::new(&backup_path).exists() {
        ...
        // Also clean up the resolved file if it exists
        let resolved_path = format!("{}.resolved", file_path);
        if Path::new(&resolved_path).exists() {
            fs::remove_file(&resolved_path).unwrap_or_else(|_| {});
        }
    }
}